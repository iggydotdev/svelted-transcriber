<script>
  ...
  let pathStyles = `animation: transitColors 1s infinite; fill: #${newHex}`,
    keyframes = "";
  ...
  onInterval(() => {
    oldHex = newHex;
    let r = toHex(~~(Math.random() * 256));
    let g = toHex(~~(Math.random() * 256));
    let b = toHex(~~(Math.random() * 256));
    newHex = `${r}${g}${b}`;
    keyframes = `@keyframes transitColors {\
                    from { fill: #${oldHex} }\
                      to { fill: #${newHex} }\
                    }`;
    pathStyles = "animation: transitColors 1s infinite;";
    document.querySelector("#styletag").innerHTML = `${keyframes}`;
  }, 1000);
</script>
...
<p>
    Built with ❤ using 
    <svg xmlns="http://www.w3.org/2000/svg" width={w} height={h}  viewBox="0 0 107 128">
    ...
    by <span>&nbsp;</span><a href='https://iggy.dev'>IggyDotDev</a>
</p>
<style>
  p {
    display: flex;
